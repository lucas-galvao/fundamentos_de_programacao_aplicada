...
import 'package:fundamentos_de_programacao_aplicada/pages/result.dart';
import 'package:fundamentos_de_programacao_aplicada/classes/model.dart';

class Loading extends StatelessWidget {
  const Loading({Key? key, required this.modelInfo}) : super(key: key);

  final Model modelInfo;

  @override
  Widget build(BuildContext context) {
    startTime(context, modelInfo);
    return Scaffold(
      ...
      body: Column(
        children: [
          _titleSection('Training the Model'),
          const SizedBox(height: 100),
          LoadingAnimationWidget.hexagonDots(color: Colors.teal, size: 100),
        ],
      ),
    );
  }
...
startTime(BuildContext context, Model model) async {
  var duration = const Duration(seconds: 5);
  return Timer(duration, () => redirect(context, model));
}

void redirect(BuildContext context, Model model) async {
  Navigator.push(context,
      MaterialPageRoute(builder: (context) => Result(modelInfo: model)));
}

Container _titleSection(String title) {
  return Container(
