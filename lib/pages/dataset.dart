...
class Dataset extends StatelessWidget {
  Dataset({Key? key}) : super(key: key);

  late Model modelInfo;
  static const List<String> listDatasets = [
    'Cifar',
    'Mnist',
    'Knist',
    'Eurosat'
  ...
  Widget build(BuildContext context) {
    ...
    if (listDatasets.contains(modelInfo.dataset)) {
      Navigator.push(
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          titleSection,
          buttonSection(context, modelInfo),
        ],
      ),
    );
  }
...
Column _buildButtonColumn(String label, BuildContext context, Model modelInfo,
  ...
  return Column(
    ...
    children: [
      ElevatedButton(
        ...
        onPressed: () {
          modelInfo.dataset = label;
          modelInfo.floor = floor;
          modelInfo.ceil = ceil;
          Navigator.push(
              context, MaterialPageRoute(builder: (context) => Grammar()));
        },
...
Column buttonSection(BuildContext context, Model modelInfo) {
  return Column(
    mainAxisAlignment: MainAxisAlignment.spaceEvenly,
    children: [
      _buildButtonColumn('Cifar', context, modelInfo, 78.3, 98.0),
      const SizedBox(height: 30),
      _buildButtonColumn('Mnist', context, modelInfo, 87.2, 99.84),
      const SizedBox(height: 30),
      _buildButtonColumn('Kmnist', context, modelInfo, 84.1, 98.6),
      const SizedBox(height: 30),
      _buildButtonColumn('Eurosat', context, modelInfo, 86.3, 99.2),
    ],
  );
}
